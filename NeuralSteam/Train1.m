function [W1,emean1] = Train1(subval)
%steps: 
...
w = 45; %#column weights in each row
L = 2; %layers, j&k
W1 = ones(L,(w+1)*Out);
dw = zeros(L,Out*(w+1));
...
%n(1:w:w*(M+1)) = n(1:w:w*(M+1))*
 n(2,1:Out:(w+1)*Out) = n(2,1:Out:(w+1)*Out)*5000;
% n(2,Out:Out:(w+1)*Out) = n(2,Out:Out:(w+1)*Out)*1000;
...
%             end
            ...
            for y = 1:Out
                yktr(tau,y) = 1/(1+exp(-aks(1,y)));
                %this gives value of yk b/w 0&1
            end
            
            for yout = 1:Out
                yk(tau,yout) = yktr(tau,yout)*Tmax(1,yout);
            end
