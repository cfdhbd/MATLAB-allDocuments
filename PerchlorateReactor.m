...
balance = @(t,val) [Kcat*Eo*Sfunc(val)/(Km+Sfunc(val));...
                    ...
                    (val(1)*32/1000*Rgas*val(2)/Va/101325+Po)*150];
                
options = odeset('Events',@events);
[tf,valf,te,ye,ie] = ode45(balance,[ti,tguess],[P0 To Po],options); 

subplot(3,1,1), plot(tf, valf(:,1)), xlabel('time (hr)'),ylabel('Product (mol)');
subplot(3,1,2), plot(tf, valf(:,2)), xlabel('time (hr)'),ylabel('Temp (K)');
subplot(3,1,3), plot(tf, valf(:,3)), xlabel('time (hr)'),ylabel('Pressure (atm)');
